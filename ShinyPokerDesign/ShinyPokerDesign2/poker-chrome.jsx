...
function ActionBar({ action, deckMode, onFold, betValue, setBetValue }) {
  const { toCall, minRaise, potForBet, heroStack, best, outs, potOdds } = action;
  ...
  return (
    <div className="actionbar">
      ...
      <div className="betcontrols" data-anno="bet">
        ...
        <div className="sliderrow">
          <div className="bslider" style={{ "--fill": fill + "%" }}>
            <input type="range" min={min} max={max} step="0.5" value={betValue}
        ...
        <div className="tbring">
          <svg viewBox="0 0 48 48"><circle cx="24" cy="24" r="20" fill="none" stroke="rgba(255,255,255,0.08)" strokeWidth="3"/>
            <circle cx="24" cy="24" r="20" fill="none" stroke="var(--accent)" strokeWidth="3" strokeLinecap="round"
              strokeDasharray="125.6" strokeDashoffset="34" transform="rotate(-90 24 24)" /></svg>
          <span className="num tnum">18</span>
        </div>
      ...
      <div className="actions" data-anno="actions">
        ...
        <button className="abtn call"><span className="key">C</span><span className="lbl">Call</span><span className="amt tnum">{toCall.toFixed(1)}</span></button>
        <button className="abtn raise"><span className="key">R</span><span className="lbl">Raise to</span><span className="amt tnum">{betValue.toFixed(betValue % 1 ? 1 : 0)}</span></button>
      </div>
    </div>
  );
}
...
function SettingsPanel({ t, set, dir, setDir, onClose }) {
    ...
    <div style={{ position: "absolute", top: 50, right: 14, zIndex: 60, width: 280, padding: "12px 14px",
      ...
      <div style={{ marginTop: 10, padding: "10px 11px", borderRadius: 8, border: "1px solid var(--accent-32)", background: "var(--accent-12)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7, fontFamily: "var(--label)", fontSize: 11, color: "var(--accent-soft)" }}>
          <span style={{ width: 14, height: 14, display: "inline-flex", flexShrink: 0 }}>{I.lock}</span> Session key active · cap 50 SOMI</div>
        <div style={{ fontFamily: "var(--label)", fontSize: 10.5, color: "var(--muted)", margin: "6px 0 9px" }}>Authorized for this table. Fold / call / raise without a wallet popup.</div>
        <button className="pill" style={{ fontSize: 11, padding: "7px 12px", width: "100%", borderColor: "rgba(229,86,42,0.4)", color: "var(--danger-soft)" }}>Revoke session key</button>
      </div>
