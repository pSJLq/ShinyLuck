...
function CashierModal({ close, addr, bal, refresh }) {
  ...
  async function refreshAll() {
    try { setWbal(Number(SP.fmt(await SP.sdk.walletBalance(), 4))); } catch {}
    try { const m = await SP.sdk.profilesFor([addr]); const p = m[addr.toLowerCase()]; setHandle(p.handle); setAv(p.avatar); setAvImg(p.img); } catch {}
    try {
      const o = await SP.sdk.roomOwner();
      const own = !!addr && o.toLowerCase() === addr.toLowerCase();
      setIsOwner(own);
      if (own) { setRake(await SP.sdk.rakeCollected()); setTrnFees(await SP.sdk.trnFeesCollected()); }
    } catch {}
    if (refresh) refresh();
  }
  uE(() => { refreshAll(); }, []);
  ...
  return (
    <Portal><div className="lt-modalbg" onClick={close}>
      <div className="lt-modal" onClick={stop} style={{ width: "min(430px,94vw)", maxHeight: "92vh", overflowY: "auto" }}>
        <h3>Cashier</h3>
        <div style={{ display: "flex", gap: 24, margin: "2px 0 10px" }}>
          <div><div style={{ fontFamily: "var(--label)", fontSize: 10, textTransform: "uppercase", letterSpacing: ".08em", color: "var(--muted)" }}>Wallet</div><div style={{ fontFamily: "var(--mono)", fontSize: 18, color: "var(--text)" }}>{wbal} {sym}</div></div>
        ...
        <label>Nickname{handle ? <> · current <b style={{ color: "var(--text)" }}>{handle}</b></> : ""}</label>
        <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
          <input value={nick} onChange={(e) => setNick(e.target.value)} placeholder={handle || "Set a nickname (3–20)"} style={{ flex: 1 }} />
          <button className="btn-sm join" disabled={busy || !nickOk} onClick={() => run(handle ? "Nickname changed" : "Nickname set", async () => { await SP.sdk.setProfile(nick.trim(), pendAv != null ? pendAv : av); if (pendAv != null) { setAv(pendAv); setPendAv(null); } bumpProfile(); })}>{handle ? "Change" : "Set"}</button>
        </div>

        <label>Avatar <Hint text="Lives fully ON-CHAIN · no server. Upload your own picture (compressed to ~96px in your browser, a few KB of gas) or pick a preset. Everyone at the table sees it." /></label>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          <AvatarIcon av={pendAv != null ? pendAv : av} img={upl ? upl.dataUri : avImg} name={handle || addr} size={56} style={{ borderRadius: 12 }} />
              ...
              <label className="btn-sm" style={{ cursor: "pointer", margin: 0 }}>
                ...
                <input type="file" accept="image/*" style={{ display: "none" }}
                  onChange={async (e) => {
                    ...
                    e.target.value = "";
                    if (!f) return;
                    try { setUpl(await SPCompressAvatar(f)); } catch (err) { flash(err.message); }
                  }} />
              </label>
              {upl && <button className="btn-sm join" disabled={busy} onClick={() => run("Avatar uploaded on-chain", async () => { await SP.sdk.setAvatarImage(upl.bytes); setAvImg(upl.dataUri); setUpl(null); bumpProfile(); })}>Upload · {(upl.bytes.length / 1024).toFixed(1)} KB</button>}
              {avImg && !upl && <button className="btn-sm" disabled={busy} onClick={() => run("Photo removed", async () => { await SP.sdk.clearAvatarImage(); setAvImg(null); bumpProfile(); })}>Remove photo</button>}
            </div>
            <span className="note" style={{ margin: 0 }}>{upl ? "Preview · press Upload to store it on-chain (~0.02 STT gas)" : avImg ? "Your photo lives on-chain · it overrides the presets below" : "Or pick a preset · free with your nickname:"}</span>
          </div>
        </div>
          ...
          {window.SP_AVATARS.map((g, id) => {
            ...
            return (
              <button key={id} type="button" disabled={busy}
                title={id === 0 ? "Auto" : g}
                onClick={() => {
                  if (handle) run("Avatar set", async () => { await SP.sdk.setProfile(handle, id); setAv(id); setPendAv(null); bumpProfile(); });
        ...
        <div className="chipset" style={{ marginBottom: 8 }}>
          <button type="button" className={tab === "deposit" ? "on" : ""} onClick={() => setTab("deposit")}>Deposit</button>
          <button type="button" className={tab === "withdraw" ? "on" : ""} onClick={() => setTab("withdraw")}>Withdraw</button>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <input value={amt} onChange={(e) => setAmt(e.target.value)} placeholder={`Amount (${sym})`} style={{ flex: 1 }} />
          {tab === "deposit"
            ? <button className="btn-sm join" disabled={busy || !amtOk} onClick={() => run("Deposit", () => SP.sdk.deposit(amt))}>Deposit</button>
            : <button className="btn-sm join" disabled={busy || !amtOk} onClick={() => run("Withdraw", () => SP.sdk.withdraw(amt))}>Withdraw</button>}
        </div>
        <p className="note">Deposit moves {sym} into your poker balance to buy in; withdraw sends idle balance back to your wallet.</p>

        <label style={{ marginTop: 6 }}>Cash out · send {sym} to another address <Hint text="Send STT out of your poker wallet to any external address (an exchange, another wallet…)." /></label>
        ...
        {isOwner && (
          <div style={{ marginTop: 10, paddingTop: 12, borderTop: "1px solid var(--line)" }}>
            <label>Owner · cash rake (10%)</label>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <span style={{ fontFamily: "var(--mono)", fontSize: 16, color: "var(--text)", flex: 1 }}>{Number(SP.fmt(rake, 6))} {sym}</span>
              <button className="btn-sm" disabled={busy || rake === 0n} onClick={() => run("Rake withdrawn", () => SP.sdk.withdrawAllRake(addr))}>Withdraw to wallet</button>
            </div>
            <label style={{ marginTop: 8 }}>Owner · tournament fees (10%)</label>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <span style={{ fontFamily: "var(--mono)", fontSize: 16, color: "var(--text)", flex: 1 }}>{Number(SP.fmt(trnFees, 6))} {sym}</span>
              <button className="btn-sm" disabled={busy || trnFees === 0n} onClick={() => run("Fees withdrawn", () => SP.sdk.withdrawAllTrnFees(addr))}>Withdraw to wallet</button>
            </div>
          </div>
        )}

        <div className="row" style={{ marginTop: 14, justifyContent: "space-between" }}>
          <button className="pill" style={{ color: "var(--danger-soft, #ef8f8f)" }} onClick={() => { if (SP.sdk.signOut) SP.sdk.signOut().finally(() => location.reload()); else location.reload(); }}>Disconnect wallet</button>
...
Object.assign(window, { CashierModal });
