    ...
    <Portal><div className="lt-modalbg" onClick={close}>
      <div className="lt-modal" onClick={stop} style={{ width: "min(430px,94vw)", maxHeight: "92vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
          ...
          <button className="btn-sm join" disabled={busy || !nickOk} onClick={() => run(handle ? "Nickname changed" : "Nickname set", async () => { await SP.sdk.setProfile(nick.trim(), pendAv != null ? pendAv : av); if (pendAv != null) { setAv(pendAv); setPendAv(null); } })}>{handle ? "Change" : "Set"}</button>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <div style={{ display: "flex", gap: 8 }}>
              ...
              {upl && <button className="btn-sm join" disabled={busy} onClick={() => run("Avatar uploaded on-chain", async () => { await SP.sdk.setAvatarImage(upl.bytes); setAvImg(upl.dataUri); setUpl(null); })}>Upload · {(upl.bytes.length / 1024).toFixed(1)} KB</button>}
              {avImg && !upl && <button className="btn-sm" disabled={busy} onClick={() => run("Photo removed", async () => { await SP.sdk.clearAvatarImage(); setAvImg(null); })}>Remove photo</button>}
            </div>
          ...
          {window.SP_AVATARS.map((g, id) => {
            ...
            return (
              <button key={id} type="button" disabled={busy}
                title={id === 0 ? "Auto" : g}
                onClick={() => {
                  if (handle) run("Avatar set", async () => { await SP.sdk.setProfile(handle, id); setAv(id); setPendAv(null); });
