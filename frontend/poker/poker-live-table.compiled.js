...
function actionDiff(prev, cur, existing, now) {
  ...
  for (const s of cur.seats) {
    ...
    if (!p) continue;
    if (sameHand && s.folded && !p.folded) acts[s.index] = stamp({
      kind: "fold"
    });else if (sameHand && s.allIn && !p.allIn) acts[s.index] = stamp({
      kind: "allin"
    });else if (sameStreet && s.committedStreet > p.cs) acts[s.index] = stamp({
      kind: prev.curBet === 0n ? "bet" : s.committedStreet > prev.curBet ? "raise" : "call",
      amt: NV(s.committedStreet)
    });else if (sameStreet && prev.actingSeat === s.index && h.actingSeat !== s.index && s.committedStreet === p.cs && !s.folded && !s.allIn && s.inHand && !committed(s.index)) acts[s.index] = stamp({
      kind: "check"
    });
    ...
    else if (sameHand && !sameStreet && prev.actingSeat === s.index && !s.folded && !s.allIn && s.inHand && !committed(s.index)) {
      const before = prev.stacks && prev.stacks[s.index];
      const paid = before != null && before > s.stack ? before - s.stack : 0n;
      const put = p.cs + paid;
      acts[s.index] = paid === 0n && p.cs >= prev.curBet ? stamp({
        kind: "check"
      }) : stamp({
        kind: put > prev.curBet ? prev.curBet === 0n ? "bet" : "raise" : "call",
        amt: NV(put > 0n ? put : prev.curBet)
      });
    }
  }
  return acts;
...
function LiveTable() {
  ...
  useEffect(() => {
    ...
    const attach = () => {
      ...
      if (ro) ro.disconnect();
      if (!el) {
        root.style.removeProperty("--sp-barh");
        root.style.removeProperty("--sp-barmax");
        max = 0;
        ro = null;
        return;
      }
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      setWrapBox(p => p && p.w === w && p.h === h && last === g.side + ":" + g.r ? p : {
        w,
        h
      });
      last = g.side + ":" + g.r;
  ...
  useEffect(() => {
    ...
    if (h.inProgress && h.handId !== prev.handId && h.handId > 0) {
      ...
      setLastActs({});
      lastActsRef.current = {}; // action badges never carry across hands
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "tnum"
  }, trn ? `${fmtChips(Number(trn.curSb))} / ${fmtChips(Number(trn.curBb))}${Number(trn.curAnte || 0) ? ` + ${fmtChips(Number(trn.curAnte))}` : ""}` : CHIPS ? `${fmtChips(NV(cfg.smallBlind))} / ${fmtChips(NV(cfg.bigBlind))}` : `${NV(cfg.smallBlind)} / ${NV(cfg.bigBlind)}`)), /*#__PURE__*/React.createElement("span", {
    className: "sub"
  }, trn ? (() => {
    const left = trn.nextAt ? Math.max(0, trn.nextAt - Math.floor(nowMs / 1000)) : null;
    const clock = trn.status !== 1 ? SPT("finished") : left == null ? SPT("final level") : left > 0 ? `${Math.floor(left / 60)}:${String(left % 60).padStart(2, "0")}` : SPT("level up next hand");
    return /*#__PURE__*/React.createElement(React.Fragment, null, SPT("Level"), " ", /*#__PURE__*/React.createElement("b", {
      className: "tnum"
    }, trn.level + 1), " \xB7 ", /*#__PURE__*/React.createElement("span", {
      className: left != null && left > 0 && left <= 30 ? "hot" : ""
    }, clock), " · ", /*#__PURE__*/React.createElement("b", {
      className: "tnum"
    }, trn.remaining), "/", trn.registered, " ", SPT("left"));
  })() : `NLHE · ${maxSeats}-max`, " · ", SPT("Hand"), " ", /*#__PURE__*/React.createElement("span", {
    className: "tnum"
  }, "#", hand.handId))), trn && trn.tables && trn.tables.length > 1 && /*#__PURE__*/React.createElement("div", {
    className: "tabsw"
  }, trn.tables.map((tid, i) => /*#__PURE__*/React.createElement("button", {
    key: tid,
    className: tid === tableId ? "on" : "",
    title: tid === tableId ? null : SPT("Switch to this table"),
    onClick: () => switchTable(tid)
  }, SPT("Table"), " ", i + 1, trn.counts && trn.counts[tid] != null ? ` · ${trn.counts[tid]}` : ""))), /*#__PURE__*/React.createElement("div", {
    className: "spacer"
